...
class BaseEvaluator(ABC):
    ...
    def evaluate_model_on_file(self, data_file: str, data_type: str) -> Tuple[pd.DataFrame, Dict]:
        ...
        prompt_lists = [[prompt] for prompt in prompts]
        ...
        for model, model_type in self.models:
            ...
            scores = model.cond_log_prob(prompt_lists, targets_lists, absolute_normalization=True)
            ...
            scores_single = [score.item() if score.dim() == 0 else score.tolist() for score in scores]
